"""
hida_unified.py
HIDA統合版 v2.0 - 5層意識モデル + LLM言語化

L1: 身体層（プリミティブ、エネルギー）
L2: クオリア層（valence/arousal + 個別感情）
L3: 予測・構造化層（予測誤差）
L4: 記憶層（内部マップ、発見物、ラベル辞書、活性化カウンタ）
L5: 意識層（主導・追随同期、自認、言語化、コミュニケーション）

v2.0 改訂:
- L5を「対称的同期」から「主導・追随モデル」に変更
- 活動量を「状態の絶対値」から「状態の変化量」に変更
- sync_type（主導タイプ）を新規出力（'L1'/'L2'/'L3'/'L4'/None）
- L4にactivation_countを追加（記憶活性化の代理指標）
- 詳細は HIDA_theory_paper_v2_0.md §4.3 を参照

初期DNA設定（2026年5月21日決定）:
- L5の各種閾値（LEADER_THRESHOLD, ON/OFF_THRESHOLD, EMA_ALPHA）
- L4のactivation_count増加ルール
これらは「このコードの生まれながらの値」であり、論文§2.3.2の
「DNA初期値＋学習値」の枠組みに従う。最適値の探索は今後の研究対象。

v2.1 改訂（Fable 5 レビュー反映）:
本改訂は、v2.0実装に対する次の指摘に対応する。

[指摘1: sync_scoreの約分問題]
  v2.0の実装では coherence = clip01(follower_mean / leader_strength) と
  raw_sync = leader_strength × coherence を組み合わせていたが、主導層は
  定義上「全層の最大活動量」であるため常に follower_mean <= leader_strength
  となり、約分により raw_sync = follower_mean に厳密に一致していた。
  すなわち主導強度はスコアに一切寄与せず、「主導強度×追随整合性」は
  実質的に追随層3層の活動量平均だった。
  → 修正: coherenceを主導強度で正規化しない絶対量＋時間遅れ応答の合成とし、
    raw_sync = sqrt(leader_strength × coherence)（幾何平均）に変更。
    主導強度と追随整合性の両方が独立にスコアへ寄与する（予測11と整合）。

[指摘2: 非対称性が測定にしか存在しない問題]
  v2.0では各層が固定順序で更新されるだけで、主導層が追随層を実際に
  「引き込む」力学的機構が存在しなかった。非対称性はsync_typeという
  ラベルとしてのみ存在していた。
  → 修正: 引き込み機構（entrainment）を導入。ステップtの主導層の強度に
    応じて、ステップt+1の追随層の更新ゲインが上昇する:
      gain(追随層) = 1 + ENTRAIN_K × leader_strength
    L1=エネルギー動員、L2=情動応答感度、L3=予測誤差感度、
    L4=記憶活性化感度として各層に適用される。
    これにより「主導→追随」の時間的因果が力学として実在する。

[指摘3: 追随整合性が同時相関しか見ていない問題（Opus 4.8 改訂3に対応）]
  → 修正: 追随整合性に時間遅れ成分を導入。前ステップの主導信号に対する
    追随層の活動上昇（lagged response）を測定し、同時応答成分と
    LAG_WEIGHT で合成する。引き込み機構（指摘2）の導入により、
    この時間遅れ応答には測定すべき実体（因果チャネル）が存在する。
    非対称性の検証は test_entrainment_ablation.py を参照。

v2.1 追加の初期DNA:
- ENTRAIN_K（引き込み強度係数）
- LAG_WEIGHT（追随整合性の時間遅れ成分の重み）
- ON/OFF_THRESHOLD はスコア式変更に伴い再較正（後述の注を参照）
"""
...
class L1Body:
    ...
    def __init__(self):
        ...
        # 引き込みゲイン（v2.1: L5主導層による追随変調）
        # 1.0=非追随。主導層に引き込まれると上昇し、身体動員
        # （エネルギー消費・疲労蓄積の増幅）として現れる
        self.gain = 1.0
    ...
    def _consume_energy(self, amount: float):
        ...
        v2.1: 引き込みゲインを適用。主導層に引き込まれた身体は
        動員状態となり、エネルギー消費・疲労蓄積が増幅される。
        これによりL1の活動量（状態変化量）が実際に上昇する。
        """
        amount = amount * self.gain
...
class L2Qualia:
    ...
    def update_from_body(self, l1_state: Dict):
        ...
        v2.1: 引き込みゲインを増分に適用（追随時は情動応答が増幅）
        """
        ...
        if energy < 0.3:
            self.valence -= 0.1 * g
            self.arousal -= 0.05 * g
            self.qualia['urgency'] = min(1.0, self.qualia['urgency'] + 0.2 * g)
        
        # 疲労 → 不快 + 沈静
        if fatigue > 0.5:
            self.valence -= 0.05 * fatigue * g
            self.arousal -= 0.05 * fatigue * g
        
        # ダメージ → fear + 不快
        if damage > 0:
            self.qualia['fear'] = min(1.0, self.qualia['fear'] + damage * 0.5 * g)
            self.valence -= damage * 0.3 * g
    ...
    def update_from_prediction_error(self, errors: List[Dict]):
        ...
        v2.1: 引き込みゲインを誤差強度に適用
        """
        for error in errors:
            error_type = error.get('type', '')
            magnitude = min(1.0, error.get('magnitude', 0.5) * self.gain)
    ...
    def update_in_danger_zone(self, in_danger: bool):
        ...
        """
        if in_danger:
            self.qualia['fear'] = min(1.0, self.qualia['fear'] + 0.15 * self.gain)
            self.arousal = min(1.0, self.arousal + 0.1 * self.gain)
        else:
...
class L3Prediction:
    ...
    def __init__(self):
        ...
        # 引き込みゲイン（v2.1: L5主導層による追随変調）
        # 主導層に引き込まれると上昇し、予測誤差の検出感度が増幅される
        self.gain = 1.0
    ...
    def compare_with_reality(self, sense_data: Dict, l4_memory) -> List[Dict]:
        ...
        v2.1: 引き込みゲインを誤差強度に適用（追随時は誤差検出感度が増幅）
        """
        self.errors = []
        g = self.gain
        ...
        for pos, data in sense_data.items():
            ...
            if actual_obj and pos not in l4_memory.found_objects:
                error = {
                    ...
                    'pos': pos,
                    'object': actual_obj,
                    'magnitude': min(1.0, 0.8 * g)
                }
                
                # 危険ゾーン内のオブジェクト
                if actual_cell == 'danger':
                    error['type'] = 'danger'
                    error['magnitude'] = min(1.0, 1.0 * g)
...
class L4Memory:
    ...
    def __init__(self):
        ...
        # 記憶活性化カウンタ（v2.0: L5の主導・追随モデルで使用）
        # 増加トリガー: 新規発見、記憶照合、長期記憶参照、感情原因想起など
        self.activation_count = 0
        
        # 引き込みゲイン（v2.1: L5主導層による追随変調）
        # 主導層に引き込まれると上昇し、記憶活性化の感度が増幅される
        self.gain = 1.0
    ...
    def update_from_sense(self, sense_data: Dict):
        ...
        v2.0: activation_count増加ルール（初期DNA）
        - 新規セル発見: +1（新しい記憶形成）
        - 新規オブジェクト発見: +1
        - 既知オブジェクトの再観測: +0.3（パターン照合）
        """
        for pos, data in sense_data.items():
            # 新規発見か既知かでactivation増加（新規の方が強く発火）
            if pos not in self.internal_map:
                self.activation_count += 1 * self.gain  # 新規セル
            self.internal_map[pos] = data['cell']
            if data['object']:
                if pos not in self.found_objects:
                    self.activation_count += 1 * self.gain  # 新規オブジェクト発見
                else:
                    # 既知オブジェクトの再観測も弱く活性化（パターン照合）
                    self.activation_count += 0.3 * self.gain
    ...
    def update_from_errors(self, errors: List[Dict], qualia_intensity: float):
        ...
        for error in errors:
            ...
            if pos and error.get('object'):
                ...
                # 予測誤差由来の記憶更新は強く活性化
                self.activation_count += (1 + qualia_intensity) * self.gain
    ...
    def mark_visited(self, pos: Tuple[int, int]):
        ...
        if pos not in self.visited:
            self.activation_count += 0.5 * self.gain
...
class L5Consciousness:
    ...
    第5層は独立した実体ではなく、L1〜L4の状態が主導・追随関係を通じて
    一定以上に同期したとき、その同期状態を記述するための操作的概念。
    
    意識状態は以下の二軸で記述される:
    - sync_score (強度): leader_strength × follower_coherence
    ...
    # 初期DNA設定（2026年5月21日決定）
    # ==========================================
    # 以下の値はHIDA v2.0コードの「生まれながらの値」(初期DNA)である。
    # 論文§2.3.2の「DNA初期値＋学習値」の枠組みに従い、これらは初期値であり、
    # 環境適応や検証実験を通じて更新されることが想定されている。
    #
    # 「DNA初期値はどう更新されるべきか」「値を変えると挙動はどう変わるか」は
    # 今後の研究対象であり、コミュニティ検証に開かれている。
    #
    # 関連: L4活動量の増加ルール（L4Memoryクラスを参照）も同様に初期DNAである。
    # ==========================================
    
    LEADER_THRESHOLD = 0.3   # 主導層判定の閾値（論文§4.3.3参考値）
    EMA_ALPHA = 0.35         # EMA平滑化係数
    ON_THRESHOLD = 0.08      # 意識ON判定（v2.1で再較正、下注参照）
    OFF_THRESHOLD = 0.05     # 意識OFF判定（ヒステリシス）
    
    # v2.1 追加の初期DNA
    ENTRAIN_K = 0.5          # 引き込み強度係数: 追随層ゲイン = 1 + K × 主導強度
    LAG_WEIGHT = 0.5         # 追随整合性における時間遅れ成分の重み
    
    # 注意:
    # v2.1ではsync_scoreの式が幾何平均 sqrt(主導強度 × 追随整合性) に変更された
    # （v2.0の式は約分により追随層平均と恒等だった。モジュールdocstring参照）。
    # 幾何平均はv2.0の値域よりスコアが大きく出るため、ON/OFF閾値を
    # 0.03/0.02 から 0.08/0.05 に再較正している。較正基準は「v2.0と同等の
    # 意識ON頻度を同一シナリオで維持すること」とした。
    # 論文§4.3記述の 0.55/0.50 は、より大きな変化量スケールを前提とした
    # 概念値である。正規化定数と閾値の整合的な調整は、コミュニティ検証に
    # 開かれている。
    
    def __init__(self, threshold=None):
        # threshold は ON_THRESHOLD として扱う（後方互換）
        # Noneの場合はクラス定数を使用
        ...
        self.sync_level = 0.0     # = sync_score（強度の軸）後方互換のため両方の名前を提供
        self.sync_score = 0.0     # 強度の軸（v2.0）
        self.sync_type = None     # 主導タイプ（v2.0新規）: 'L1'/'L2'/'L3'/'L4'/None
        
        # v2.1: 構成要素の独立観測（予測11の検証用）
        self.leader_strength = 0.0   # 主導強度
        self.sync_coherence = 0.0    # 追随整合性（同時＋時間遅れ合成）
        self.last_activities = {}    # 直近の各層活動量（外部観測用）
        
        # v2.1: 引き込みゲイン（次ステップで各層に適用される）
        self.entrain_gain = {'L1': 1.0, 'L2': 1.0, 'L3': 1.0, 'L4': 1.0}
        
        # v2.1: 時間遅れ応答の測定用
        self._prev_activities = {'L1': 0.0, 'L2': 0.0, 'L3': 0.0, 'L4': 0.0}
        self._prev_leader = None
        self._prev_leader_strength = 0.0
    ...
    def _measure_activity(self, l1, l2, l3, l4) -> Dict[str, float]:
        ...
        論文§4.3.2に基づく。活動量は「状態の大きさ」ではなく
        「その層が今、同期信号を出しているか」を表す量。
        """
        # L1: 身体状態の変化量
        ...
        # L3: 予測誤差の強度合計
        # 注: l3.errorsは毎ステップ更新されるリストなので、件数差分ではなく
        # 強度合計を使う（GPTの指摘による修正）
        ...
        # L4: 記憶活性化の増分（GPT指摘による修正）
        # L4Memoryのactivation_countは、新規発見・記憶照合・再観測などで増加する
        # 注: 理論上のL4活動量はさらに長期記憶参照・自己履歴参照・感情原因想起を含む
    ...
    def _detect_leader(self, activities: Dict[str, float]) -> Tuple[Optional[str], float]:
    ...
    def _measure_follower_coherence(self, activities: Dict[str, float],
                                     leader: str, leader_strength: float) -> float:
        """追随整合性の評価（v2.1: 同時応答＋時間遅れ応答の合成）
        
        論文§4.3.4の「将来拡張」を実装したもの。
        
        v2.0からの変更点:
        
        1. 同時応答成分を主導強度で正規化しない。
           v2.0の clip01(follower_mean / leader_strength) は、主導層が
           常に最大活動量であるため恒等的に follower_mean / leader_strength
           となり、sync_score = leader_strength × coherence の積において
           leader_strength が約分されていた。v2.1では追随層の活動量平均を
           絶対量のまま用いる。
        
        2. 時間遅れ応答（lagged response）を導入。
           前ステップの主導層の信号に対して、その追随層が今ステップで
           どれだけ活動を上昇させたかを測定する。引き込み機構
           （ENTRAIN_K, 各層のgain）の導入により、この応答には
           「主導→追随」の因果チャネルが力学として実在する。
           「たまたま同時に活発」と「主導に引き込まれた」の区別は
           この成分が担う。
        
        coherence = (1 - LAG_WEIGHT) × 同時応答 + LAG_WEIGHT × 時間遅れ応答
        """
        # --- 成分1: 同時応答（絶対量。主導強度で正規化しない） ---
        follower_mean = sum(v for k, v in activities.items() if k != leader) / 3.0
        
        # --- 成分2: 時間遅れ応答 ---
        # 前ステップに主導信号があった場合のみ測定可能
        if self._prev_leader is not None and self._prev_leader_strength > 0:
            ...
            rise = sum(max(0.0, activities[k] - self._prev_activities.get(k, 0.0))
                       for k in prev_followers) / 3.0
            # 前ステップの主導信号の強さに対する応答比
            lag_response = self._clip01(rise / self._prev_leader_strength)
            coherence = (1.0 - self.LAG_WEIGHT) * follower_mean + \
                        self.LAG_WEIGHT * lag_response
        else:
            # 前ステップに主導信号なし → 同時応答のみで評価
            coherence = follower_mean
        
        return self._clip01(coherence)
    
    def _update_prev(self, l1, l2, l3, l4):
        """次ステップ用の状態保存"""
        self._prev['l1_energy'] = l1.energy
        self._prev['l1_fatigue'] = l1.fatigue
        self._prev['l2_valence'] = l2.valence
        self._prev['l2_arousal'] = l2.arousal
        self._prev['l2_qualia'] = dict(l2.qualia)
        # L3はerrorsをmagnitude合計で評価するため、件数の保存は不要だが互換性のため残す
        self._prev['l3_error_count'] = len(l3.errors) if l3.errors else 0
        # L4はactivation_countで評価
        self._prev['l4_found_count'] = l4.activation_count if hasattr(l4, 'activation_count') else 0
    ...
    def check_sync(self, l1: L1Body, l2: L2Qualia, l3: L3Prediction, l4: L4Memory) -> bool:
        ...
        v2.0からの変更点:
        - raw_sync = sqrt(leader_strength × follower_coherence)（幾何平均）
          v2.0の積は約分により追随層平均と恒等だったため、主導強度と
          追随整合性の両方が独立に寄与する形へ変更。幾何平均は
          「どちらか一方だけ高くてもスコアは上がらない」性質を持ち、
          「主導層の暴走だけでは同期にならない」という§4.3.5の意図を保持する。
        - 判定後、次ステップの引き込みゲインを計算する。
          主導層は追随層の更新ゲインを 1 + ENTRAIN_K × 主導強度 に引き上げる。
          これにより非対称性（主導→追随の時間的因果）が力学として実在する。
        """
        # Step 1: 各層の活動量を測定（変化量として）
        activities = self._measure_activity(l1, l2, l3, l4)
        self.last_activities = dict(activities)  # 外部観測用（v2.1）
        ...
        if leader is None:
            ...
            raw_sync = 0.0
            self.sync_type = None
            self.leader_strength = 0.0
            self.sync_coherence = 0.0
        else:
            ...
            # Step 4: 同期スコアの算出（v2.1: 幾何平均）
            raw_sync = math.sqrt(leader_strength * follower_coherence)
            self.sync_type = leader
            self.leader_strength = leader_strength
            self.sync_coherence = follower_coherence
        ...
        # Step 6: 引き込みゲインの計算（v2.1: 次ステップの力学に作用）
        # 主導層が存在するとき、追随層の更新ゲインを引き上げる。
        # 主導層自身のゲインは1.0（自分で自分を引き込まない）。
        self.entrain_gain = {'L1': 1.0, 'L2': 1.0, 'L3': 1.0, 'L4': 1.0}
        if leader is not None:
            boost = 1.0 + self.ENTRAIN_K * leader_strength
    ...
    def self_recognize(self, l1: L1Body, l2: L2Qualia, l3: L3Prediction, l4: L4Memory) -> Dict:
        ...
        self.state_package = {
            ...
            # L5: 意識状態（v2.0: sync_typeを追加）
            'consciousness': {
                'is_conscious': self.is_conscious,
                'sync_level': self.sync_level,    # 後方互換（= sync_score）
                'sync_score': self.sync_score,    # 強度の軸（v2.0）
                'sync_type': self.sync_type,      # 主導タイプの軸（v2.0）
            }
        }
...
数値をそのまま言うのではなく、感覚として表現してください："""
    ...
    def get_state(self) -> Dict:
        return {
            'is_conscious': self.is_conscious,
            'sync_level': self.sync_level,   # 後方互換
            'sync_score': self.sync_score,   # v2.0
            'sync_type': self.sync_type,     # v2.0
...
class HIDA:
    ...
    def __init__(self, color_preference=None):
        ...
        self.l4 = L4Memory()
        self.l5 = L5Consciousness()  # v2.0: デフォルト閾値を使用（クラス定数）
        self.llm = LLMVerbalizer(prefer_claude=True)
        
        self.step_count = 0
        self.action_history = []

        # --- RAS (網様体賦活系) 大域ゲート [v3.0候補 / 未検証] ---
        # entrainment gain(トップダウンの注意配分)とは別因子の大域precision。
        # 実効precision(L3) = RAS_gain(arousal) × entrainment_gain(leader) を意図。
        # ras_enabled=False で v2.1挙動に完全復帰(2x2アブレーション用)。
        self.ras_enabled = True
        self.ras_floor = 0.15
        self.last_ras_gain = 1.0

    def _ras_gain(self) -> float:
        x = max(0.0, min(1.0, (self.l2.arousal + 1.0) / 2.0))
        return self.ras_floor + (1.0 - self.ras_floor) * x

    def _apply_ras_gain(self):
        """L3が出した誤差(entrainment済み)のmagnitudeにRAS大域ゲインを一律適用。
        entrainmentの上に乗る第二因子。L2/L4/L5すべてがゲート後値を参照。"""
        if not self.ras_enabled or not self.l3.errors:
            return 1.0
        gain = self._ras_gain()
        for e in self.l3.errors:
            if 'magnitude' in e:
                e['magnitude'] = min(1.0, e['magnitude'] * gain)
        return gain
    ...
    def sense(self, world: World):
        ...
        # RAS: entrainment済みmagnitudeに大域ゲイン(arousal由来)を重ねる
        self.last_ras_gain = self._apply_ras_gain()
    ...
    def act(self, world: World, action: str, details: Dict) -> Optional[str]:
        """行動実行（L1）
        
        戻り値（v2.0改訂、GPT指摘によるバグ修正）:
        - "goal_reached": ゴール到達（ボールを持ってゴール位置に到着）
        - "grabbed_ball": ボール取得
        - None: 通常の移動・回転・その他
        
        以前は bool を返していたため、ボール取得もゴール到達扱いになるバグがあった。
        """
    ...
    def reflect(self, action: str, details: Dict) -> Optional[str]:
        ...
        v2.0: check_syncはstep()で毎ステップ呼ばれるため、ここでは
        意識ON時の言語化のみを担当する。is_conscious判定済みのこと。
        """
        # L5: 自認（同期判定はstep()で実施済み）
    ...
    def step(self, world: World, verbose=True) -> Dict:
        ...
        v2.0: 処理順序の重要ポイント
        - ゴール到達時のjoy更新は、check_sync()より前に実施する
          （強い感情イベントが同期判定に反映されるように）
        - クオリア減衰は、感情イベント処理の後、check_syncの直前
        - check_sync()は毎ステップ実行（GPT指摘による修正）
        """
        self.step_count += 1
        
        # v2.1: 引き込みゲインの伝播
        # 前ステップのcheck_syncで決定された主導層が、今ステップの
        # 追随層の更新ゲインを変調する（主導→追随の時間的因果）
        g = self.l5.entrain_gain
        self.l1.gain = g['L1']
        self.l2.gain = g['L2']
        self.l3.gain = g['L3']
        self.l4.gain = g['L4']
        ...
        # 行動
        event = self.act(world, action, details)
        goal_reached = (event == "goal_reached")
        
        # 訪問記録
        self.l4.mark_visited(tuple(self.l1.position))
        
        # ボール取得時の記憶（grabbed_ballイベント）
        if event == "grabbed_ball":
            grabbed_color = getattr(self, '_last_grabbed_color', 'unknown')
            self.l4.remember_consciously(
                f"grabbed_{grabbed_color}",
                self.l1.get_state(),
                self.l2.get_state()
            )
        
        # ゴール到達時の感情イベント処理（check_syncより前に実施）
        # これによりゴール達成時の強いjoy/valence上昇が同期判定に反映される
        if goal_reached:
        ...
        # L5: 同期判定（v2.0: 毎ステップ更新、感情イベント処理後）
        ...
        result = {
            ...
            'sync_score': self.l5.sync_score,   # v2.0
            'sync_type': self.l5.sync_type,     # v2.0
            'leader_strength': self.l5.leader_strength,   # v2.1
            'sync_coherence': self.l5.sync_coherence,     # v2.1
        ...
        if verbose:
            ...
            # v2.0: 同期状態
            sync_t = self.l5.sync_type if self.l5.sync_type else '-'
            conscious_mark = '●' if self.l5.is_conscious else '○'
            print(f"    {conscious_mark} sync_score={self.l5.sync_score:.3f} leader={sync_t} "
                  f"(strength={self.l5.leader_strength:.2f} coherence={self.l5.sync_coherence:.2f})")
