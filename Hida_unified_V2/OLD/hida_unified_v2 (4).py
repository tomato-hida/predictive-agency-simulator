        ...
        for pos, data in sense_data.items():
            ...
            if data['object']:
                if pos not in self.found_objects:
                    self.activation_count += 1  # 新規オブジェクト発見
                else:
                    # 既知オブジェクトの再観測も弱く活性化（パターン照合）
                    self.activation_count += 0.3
...
class L5Consciousness:
    ...
    # 注意:
    # 現在の活動量スケール（変化量ベース、各層0-1正規化）では raw_sync が小さく出るため、
    # デモ版では ON/OFF 閾値を 0.03/0.02 に調整している。
    # 論文§4.3記述の 0.55/0.50 は、より大きな変化量スケールを前提とした概念値である。
    # 各層の正規化定数と閾値の整合的な調整は、コミュニティ検証に開かれている。
    ...
    def _measure_follower_coherence(self, activities: Dict[str, float],
        ...
        論文§4.3.4に基づく。leaderを除く3層の活動量平均を
        leader_strengthで正規化する。
        
        注: 簡易版であり、「同時に活発」と「主導に引き込まれた」を
        区別できない。将来は時間遅れ相関への拡張が望ましい。
        """
