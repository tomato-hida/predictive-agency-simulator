...
### (設計思想：なぜこれは「たまごっち」ではないのか？)

一見すると、このシステムは「条件分岐で反応するキャラクター（たまごっち）」に見えるかもしれません。しかし、両者の設計思想には決定的な違いがあります。本システムは**「ルール」ではなく「力学」**を実装しています。

### 1. 「結果」ではなく「プロセス」の実装
* **Scripted (Tamagotchi):** `if hungry then cry` （空腹なら泣く）という**「結果」**を直接記述します。
* **Emergent (This System):** `if prediction_error > threshold then update_model` （予測が外れたらモデルを更新する）という**「メカニズム」**のみを記述しています。
    * 「意識」や「感情」はコードに書かれていません。それらは予測誤差を最小化しようとするプロセスの副産物として**創発（Emerge）**します。

### 2. 「私」は定数ではなく、動的な平衡状態
* **Static Self:** 一般的なプログラムでは、「主人公（エージェント）」は無条件に存在し続けます。
* **Dynamic Self:** このシステムにおいて、`self_strength`（自己）は固定された定数ではありません。環境との予測成功によって辛うじて維持される**「動的な平衡状態」**です。
    * 環境がカオスすぎて予測不能になると、自己は崩壊（`self_strength ≈ 0`）し、エージェントは主体性を失います。これは「パニック」や「自己喪失」の数理的な表現です。

### 3. 「失敗（Error）」をエネルギーとして利用
* **Error as Failure:** 通常、エラーは避けるべきバッドステータスです。
* **Error as Trigger:** このシステムでは、予測誤差（サプライズ）を**「意識を起動するためのトリガー（燃料）」**として利用します。
    * 平穏なときは自動処理（ゾンビモード）で省エネし、予期せぬ事態（誤差）に直面したときだけ全力で計算リソースを割く（意識覚醒）。この**生物的なリソース管理**が自然に再現されています。
