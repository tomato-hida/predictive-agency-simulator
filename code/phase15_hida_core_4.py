...
class Memory:
    ...
    action_recipes: Dict = None  # 行動レシピ {"片付ける": [...], "数える": [...]}
    ...
    def __post_init__(self):
        ...
        if self.preferences is None:
            self.preferences = {
                ...
                "custom_actions": {},  # カスタム行動
            }
        if self.action_recipes is None:
            self.action_recipes = dict(self.DEFAULT_RECIPES)
    
    def save(self, filepath: str = MEMORY_FILE):
        """記憶をJSONに保存"""
        data = {
            "preferences": self.preferences,
            "action_recipes": self.action_recipes,
            "original_pos": self.original_pos,
            "experiences": self.experiences[-100:],  # 最新100件だけ
            "success_rate": self.success_rate,
        }
        try:
            with open(filepath, 'w', encoding='utf-8') as f:
                json.dump(data, f, ensure_ascii=False, indent=2)
            print(f"  [記憶保存] {filepath}")
    ...
    def load(self, filepath: str = MEMORY_FILE):
        ...
        try:
            ...
            if "action_recipes" in data:
                # デフォルトレシピを保持しつつ、保存されたレシピを追加
                for name, recipe in data["action_recipes"].items():
                    if name not in self.DEFAULT_RECIPES:
                        self.action_recipes[name] = recipe
            if "original_pos" in data:
                self.original_pos.update(data["original_pos"])
            if "experiences" in data:
                self.experiences = data["experiences"]
            if "success_rate" in data:
                self.success_rate.update(data["success_rate"])
            
            print(f"  [記憶読込] {filepath}")
            print(f"    - verbs: {len(self.preferences.get('verbs', {}))}件")
            print(f"    - responses: {len(self.preferences.get('responses', {}))}件")
            print(f"    - custom_actions: {len(self.preferences.get('custom_actions', {}))}件")
            print(f"    - experiences: {len(self.experiences)}件")
            return True
        except Exception as e:
            print(f"  [記憶読込エラー] {e}")
            return False
...
class HidaCore:
    ...
    def _handle_answer(self, text: str):
        ...
        # キャンセル処理
        if any(word in text for word in ["キャンセル", "やめ", "間違", "違う", "やっぱ"]):
            print(f"  → キャンセルしました")
            self.waiting_for_answer = False
    ...
    def _parse_command(self, text: str):
        ...
        # ===== 「忘れて」で学習を削除 =====
        if "忘れて" in text or "消して" in text:
            # 何を忘れるか
            forgot_something = False
            
            # custom_actions から探す
            for keyword in list(self.memory.preferences.get("custom_actions", {}).keys()):
                if keyword in text:
                    del self.memory.preferences["custom_actions"][keyword]
                    print(f"  → [忘却] 「{keyword}」の行動を忘れました")
                    forgot_something = True
                    break
            
            # responses から探す
            if not forgot_something:
                for keyword in list(self.memory.preferences.get("responses", {}).keys()):
                    if keyword in text:
                        del self.memory.preferences["responses"][keyword]
                        print(f"  → [忘却] 「{keyword}」の応答を忘れました")
                        forgot_something = True
                        break
            
            # verbs から探す
            if not forgot_something:
                for keyword in list(self.memory.preferences.get("verbs", {}).keys()):
                    if keyword in text:
                        del self.memory.preferences["verbs"][keyword]
                        print(f"  → [忘却] 「{keyword}」の動詞を忘れました")
                        forgot_something = True
                        break
        ...
        # ===== 行動レシピをチェック =====
        for recipe_name in self.memory.action_recipes.keys():
            if recipe_name in text:
                print(f"  → [L4] 行動レシピ「{recipe_name}」を実行")
                self._execute_custom_action(recipe_name, recipe_name)
                return
    ...
    def _execute_custom_action(self, keyword: str, action_desc: str):
        ...
        # まずレシピを探す
        recipe = self.memory.action_recipes.get(keyword)
        if recipe:
            print(f"  [レシピ発見] {keyword}")
            self._execute_recipe(recipe)
            return
        
        # レシピがなければ旧ロジック（文字列マッチ）
        # 「数える」「何個」が含まれていたらボールを数える
        if "数え" in action_desc or "何個" in action_desc or "カウント" in action_desc:
            self._primitive_count("all_balls")
            self._primitive_report("count_result")
        
        # 「片付け」が含まれていたら全部戻す
        elif "片付け" in action_desc or "戻す" in action_desc or "元に" in action_desc:
            self._execute_recipe(self.memory.action_recipes.get("片付ける", []))
        
        # 「報告」が含まれていたら状態を報告
        elif "報告" in action_desc or "状態" in action_desc:
            self._primitive_report("status")
    ...
    def _execute_recipe(self, recipe: List[Dict]):
        """レシピを実行"""
        for step in recipe:
            primitive = step.get("primitive")
            
            if primitive == "count":
                self._primitive_count(step.get("target"))
            
            elif primitive == "report":
                self._primitive_report(step.get("content"))
            ...
            elif primitive == "for_each":
                target = step.get("target")
                do_steps = step.get("do", [])
                
                # 対象を取得
                items = self._get_items(target)
                
                # 各アイテムに対して実行
                for item in items:
                    self._current_item = item
                    print(f"  [for_each] {item}")
                    ...
                    for do_step in do_steps:
                        ...
                        if prim == "fetch":
                            ...
                            if pos:
                                self.action_queue.append({"type": "put_back", "target": item})
                        elif prim == "put_down":
                            pass  # put_back に含まれる
                ...
                if self.action_queue:
                    ...
                    self.consciousness = ConsciousnessState.SEARCHING
                    print(f"  [キュー] {len(self.action_queue)}件のタスクを追加")
    
    def _get_items(self, target: str) -> List[str]:
        """対象のアイテムリストを取得"""
        if target == "all_balls":
            return [obj for obj in self.memory.objects.keys() if "ボール" in obj]
        elif target == "all_objects":
            return list(self.memory.objects.keys())
        else:
            return [target]
    
    def _primitive_count(self, target: str):
        """プリミティブ: 数える"""
        items = self._get_items(target)
        self._last_count = len(items)
        print(f"  [count] {target} = {self._last_count}")
    
    def _primitive_report(self, content: str):
        """プリミティブ: 報告"""
        if content == "count_result":
            count = getattr(self, '_last_count', 0)
            print(f"  飛騨: 「{count}個あります」")
        elif content == "status":
...
def main():
    print("=" * 60)
    print("Phase 15: HIDA Core Architecture - Action Learning")
    print("飛騨アーキテクチャ本体 + 行動学習")
    print("=" * 60)
    print()
    print("L1〜L5 が常時ループ")
    print("名前を呼ぶと意識 ON")
    ...
    try:
        while True:
            hida.waiting_input = True  # 入力待ち開始
            text = input("\nあなた: ").strip()
            hida.waiting_input = False  # 入力待ち終了
            if text.lower() == 'q':
                break
            if text:
                hida.hear(text)
