...
def explore_room(hida, world, max_steps=200, wall_move_prob=0.05):
    ...
    for step in range(max_steps):
        ...
        # ミッション対象があるか
        has_mission_target = (ball_pos and not hida.holding) or (hida.holding and goal_pos)
        
        # クオリアで優先度判定
        if q['desire'] > 0.6 and has_mission_target:
            # desire高い + ターゲットあり → ミッション優先
            if unexplored and not exploration_done:
                print(f"  💡 desire={q['desire']:.2f} → ミッション優先！")
        ...
        elif unexplored and not exploration_done:
            # curiosity優先 → 探索続行
