"""
world.py
グリッドワールド - 神は全部見える、HIDAは前方1マスだけ
"""

class World:
    def __init__(self, size=5, height=None):
        self.width = size
    ...
    def see_cell(self, pos):
        ...
        if self.grid[y][x] == 'danger':
            ...
            if (x, y) in self.objects:
                ...
                return {'type': 'danger', 'name': obj['name'], 'color': obj['color']}
            return {'type': 'danger'}
    ...
    def display(self, internal_map=None):
                ...
                if [x, y] == self.hida_pos:
                    ...
                    if (x, y) in self.objects:
                        ...
                        if obj.get('color') == 'red':
                            row += "[r]"  # 危険ゾーンでもボールは優先表示
                        elif obj.get('color') == 'blue':
                            row += "[b]"
                        else:
                            row += "[!]"
                    else:
                    ...
                    if obj.get('color') == 'red':
                        ...
                        row += "[G]"
                    elif obj.get('name') == 'goal_red':
                        row += "[R]"
                    elif obj.get('name') == 'goal_blue':
                        row += "[B]"
                    else:
                        row += "[o]"
                else:
        ...
        if self.hida_holding:
            print(f"持ってる: {self.hida_holding['name']} ({self.hida_holding.get('color', '')})")
