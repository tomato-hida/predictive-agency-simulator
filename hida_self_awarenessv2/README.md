...
### 目的駆動
- 目的がないと動かない（`no goal = no action`）
- 原動力は「目的」、予測誤差は「フィードバック」

### 予測と誤差計算
- 行動前に予測を生成（「red_ballまで残り2マス」など）
- 行動後に予測と実際を比較して誤差計算

### クオリア→行動影響
- `frustration > 0.5` → 別のアプローチを試す
- `comfort < 0.3` → waitを避けて動き出す
- `curiosity > 0.5` → 探索ノイズ（ただし目的達成中は抑制）

### 3歳児問題の修正
- 何か持っている間は好奇心を抑制
- 以前：ボール持ってても好奇心で離す（3歳児）
- 修正後：目的達成中は目的優先（大人）
