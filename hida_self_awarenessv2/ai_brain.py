...
出力形式（JSON）：
{
  ...
  "self_awareness": "今の自分の状態（シンプルに）"
    ...
    def _decide_by_rule(self, state_json, world):
        ...
        memory = state_json.get('L4_memory', {})
        ...
        # ========== 自己パターンを取得（Step 4）==========
        self_pattern = memory.get('self_pattern', {})
        comfort_threshold = self_pattern.get('comfort_threshold', 0.3)
        curiosity_tendency = self_pattern.get('curiosity_tendency', 0.5)
        frustration_tolerance = self_pattern.get('frustration_tolerance', 0.5)
        exploration_rate = self_pattern.get('exploration_rate', 0.0)
        wall_collision_rate = self_pattern.get('wall_collision_rate', 0.0)
        
        # ========== 壁回避学習 ==========
        # 壁が前にある場合、過去の衝突経験から学習
        wall_ahead = front_cell and (front_cell.startswith('wall') if isinstance(front_cell, str) else False)
        if wall_ahead and 'move_forward' in legal_actions:
            # wall_collision_rate が高いほど壁を避ける
            # 最初(0.0)は100%ぶつかりに行く、学習が進むと避ける
            avoid_probability = wall_collision_rate
            if random.random() < avoid_probability:
                # 学習済み：壁を避ける（move_forward を選択肢から外す）
                legal_actions = [a for a in legal_actions if a != 'move_forward']
            # else: まだ学習中、ぶつかって学ぶ
        ...
        # frustration高 → 別のアプローチ試す
        # 自己パターン：frustration_toleranceが高いほど我慢できる
        frust_threshold = max(0.3, frustration_tolerance)
        if frustration > frust_threshold and is_conscious:
            if random.random() < frustration * 0.3:
        ...
        # comfort低 → より積極的に動く
        # 自己パターン：comfort_thresholdで判定（学習した閾値）
        avoid_wait = comfort < comfort_threshold
        
        # curiosity高 → 探索ノイズ
        # 自己パターン：exploration_rateが高いほど探索しやすい
        # ただし、目的達成中（何か持ってる）は抑制
        goal_in_progress = holding is not None
        
        # 探索確率 = ベース10% + exploration_rate × 10%
        explore_prob = 0.1 + (exploration_rate * 0.1)
        # curiosity閾値 = 0.5 - curiosity_tendency × 0.2（探索好きなら閾値下がる）
        curiosity_threshold = max(0.3, 0.5 - (curiosity_tendency - 0.5) * 0.4)
        
        if is_conscious and curiosity > curiosity_threshold and not goal_in_progress:
