...
変調値（hida_modulation.json）
  ...
  - last_danger_count: 処理済みdanger回数（差分計算用）
  - last_safe_count: 処理済みsafe成功回数
  - last_red_count: 処理済みred成功回数
    ↓
L2クオリアに適用
    ↓
スコア計算が変わる
...
energy_caution *= 0.95   # 5%減衰（最低0.0）

# 2. 新規経験の加算（前回からの差分のみ）
fear_weight += new_danger_count * 0.1      # 新しい痛み1回あたり+0.1
safe_preference += new_safe_success * 0.05 # 安全成功1回あたり+0.05
safe_preference -= new_red_success * 0.02  # 赤成功1回あたり-0.02
```

**なぜ差分加算か：**
```
旧方式: LTM全件から毎回再計算 → 忘却を上書き → 永遠に忘れない
新方式: 保存値に忘却 → 新規分だけ加算 → 経験しなければ本当に薄れる
```

**効果：**
```
痛い目に遭う → fear_weight上昇 → 慎重になる
安全が続く   → fear_weight減衰 → また冒険的になる
再び痛い目   → fear_weight再上昇 → 思い出す
