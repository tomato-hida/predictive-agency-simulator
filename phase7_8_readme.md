...
本当の「親」を作るには何が必要か。3つのアプローチがある。

**A. 共有辞書（教示）を最初に渡す**

親も最初は教える。LLM にも最初に対応規則を渡す。

```
・qualia_value がこの範囲なら「痛い」
・emotion がこの範囲なら「安心/不安」
・self_strength がこの範囲なら「自己が弱い/強い」
```

その後は数値だけでも、LLM は "学習済みの辞書" としてラベル付けできる。
これは「言語は継承」という考えと一致する。

→ 今すぐ実装可能

**B. HIDA 側にラベル生成器を持たせる**

ラベル付けは HIDA の内部機能（離散化）にして、LLM は文章整形だけに役割分担する。
これも人間っぽい（脳内でカテゴリ化してから言語化する）。
...
という行動の結果があれば、LLM は "親" に近づける。
状態と行動の関係から意味を学習できる。

→ フィジカル AI への道、実験規模が上がる
